<div class="bg-gray-50 min-h-screen">
    @php
        ...
        $btnBlk = 'px-3 py-2 text-xs font-medium rounded-lg bg-gray-900 text-white hover:bg-black focus:outline-none focus:ring-2 focus:ring-gray-900/20 transition';
        ...
        $vehicleName = $vehicleMap[$booking->vehicle_id] ?? 'Unknown';
    @endphp
    ...
    <main class="px-4 sm:px-6 py-6 space-y-8">
            ...
            <div class="relative p-6 sm:p-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-lg sm:text-xl font-semibold">
                                Booking #{{ $booking->vehiclebooking_id }}
                            </h2>
                            <p class="text-sm text-white/80">
                                Purpose: <span class="font-semibold text-white">{{ $booking->purpose ?? 'N/A' }}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 ml-auto mt-4 md:mt-0">
                        ...
                        <a href="{{ url()->previous() }}" class="{{ $btnLt }} border-white/30 text-white/90 hover:bg-white/10">Back</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6 lg:col-span-1 lg:order-2">
                ...
                <div class="{{ $card }}">
                    <div class="px-5 py-4 border-b border-gray-200">
                        <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider">Vehicle & Borrower</h3>
                    </div>
                    ...
                    <div class="p-5 space-y-4 text-sm">
                        ...
                        <div class="group flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-white shadow-sm hover:border-indigo-200 transition-colors">
                            ...
                            <div class="flex flex-col overflow-hidden min-w-0">
                                <span class="text-xs text-gray-400 font-medium mb-0.5">Vehicle</span>
                        ...
                        <div class="grid grid-cols-1 gap-3 pt-2 border-t border-gray-100">
                            ...
                            <div class="flex flex-wrap items-center justify-between">
                                <p class="text-xs text-gray-500">Admin Approval</p>
                            ...
                            <div class="flex flex-wrap items-center justify-between">
                                <p class="text-xs text-rose-600">Soft-Deleted</p>
                ...
                <section class="{{ $card }}">
                    <div class="px-5 py-4 border-b border-gray-200">
                        <h3 class="text-base font-semibold text-gray-900">Before Photos ({{ $beforePhotos->count() }})</h3>
                    </div>
                    <div class="p-5">
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            <div class="relative group border rounded-lg overflow-hidden bg-gray-50 cursor-pointer"
                                ...
                                <img src="{{ $photoUrl($p->photo_path) }}" alt="before" class="w-full h-24 object-cover">
                                @if($p->deleted_at)
                                <span class="absolute top-1 left-1 text-[9px] bg-rose-600 text-white px-1 py-0.5 rounded">Deleted</span>
                                @endif
                            </div>
                            @empty
                            <div class="col-span-full text-center text-sm text-gray-500 p-3">No Before Photos</div>
                            @endforelse
                        </div>
                    </div>
                </section>
                ...
                <section class="{{ $card }}">
                    <div class="px-5 py-4 border-b border-gray-200">
                        <h3 class="text-base font-semibold text-gray-900">After Photos ({{ $afterPhotos->count() }})</h3>
                    </div>
                    <div class="p-5">
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            <div class="relative group border rounded-lg overflow-hidden bg-gray-50 cursor-pointer"
                                wire:click="openPhotoPreview('{{ $photoUrl($p->photo_path) }}')">
                                <img src="{{ $photoUrl($p->photo_path) }}" alt="after" class="w-full h-24 object-cover">
                                @if($p->deleted_at)
                                <span class="absolute top-1 left-1 text-[9px] bg-rose-600 text-white px-1 py-0.5 rounded">Deleted</span>
                                @endif
                            </div>
                            @empty
                            <div class="col-span-full text-center text-sm text-gray-500 p-3">No After Photos</div>
                            @endforelse
                        </div>
                    </div>
                </section>
            ...
            <section class="lg:col-span-2 lg:order-1 space-y-6">
                ...
                <div class="{{ $card }}">
                    <div class="px-5 py-4 border-b border-gray-200">
                        <h2 class="text-base font-semibold text-gray-900">Booking Schedule & Destination</h2>
                    </div>
                    <div class="p-5 space-y-4">
                        ...
                        <div>
                            <label class="{{ $label }}">Notes / Reject Reason</label>
