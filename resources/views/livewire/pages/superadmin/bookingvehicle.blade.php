<div class="bg-gray-50 min-h-screen"> @php use Carbon\Carbon; use Illuminate\Support\Facades\Storage;
        ...
        <div class="relative z-0 p-6 sm:p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-lg sm:text-xl font-semibold">Manajemen Riwayat Kendaraan</h2>
                ...
                <div class="flex items-center gap-3">
                    <label class="inline-flex items-center gap-2 text-sm text-white/90 cursor-pointer">
                        ...
                        <span>Termasuk yang Dihapus</span>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="flex-1">
                <p class="text-xs text-gray-600 font-medium">
                    ...
                    Kendaraan: {{ $vehicleMap[$vehicleFilter] ?? '—' }}
                    @else
                    Menampilkan Data {{ $statusTab === 'rejected' ? 'Ditolak' : 'Selesai' }}
                    @endif
                </p>
                <p class="text-xs text-gray-400 mt-1">
                    {{ $q ? 'Mencari: ' . $q : 'Ketuk filter untuk menyesuaikan pengaturan.' }}
                </p>
            </div>
            ...
            <div class="{{ $card }}">
                <div class="px-4 sm:px-6 pt-4 pb-3 border-b border-gray-200 hidden lg:block">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label class="{{ $label }}">Cari Tujuan/Peminjam</label>
                            <div class="relative">
                                <input type="text" wire:model.live.debounce.400ms="q" placeholder="Cari tujuan, destinasi, peminjam…"
                        ...
                        <div>
                            <label class="{{ $label }}">Filter per Kendaraan</label>
                        ...
                        <div>
                            <label class="{{ $label }}">Filter per Tanggal</label>
                            <div class="relative">
                        ...
                        <div>
                            <label class="{{ $label }}">Urutkan & Bersihkan</label>
                                ...
                                <div class="flex items-start gap-3 flex-1 min-w-0">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-[12px] text-gray-500">
                                            Kendaraan: <span class="font-medium text-gray-700 truncate">{{ $vehicleName }}</span>
                                        </p>

                                        <div class="flex flex-wrap items-center gap-2 mt-2 text-xs">
                                            <span class="{{ $chip }}"><span class="text-gray-500">Peminjam:</span><span class="font-medium text-gray-700">{{ $b->borrower_name ?? 'N/A' }}</span></span>
                                            <span class="{{ $chip }}"><span class="text-gray-500">Dari:</span><span class="font-medium text-gray-700">{{ fmtDate($b->start_at) }} {{ fmtTime($b->start_at) }}</span></span>
                                            <span class="{{ $chip }}"><span class="text-gray-500">Sampai:</span><span class="font-medium text-gray-700">{{ fmtTime($b->end_at) }}</span></span>
                                ...
                                <div class="flex flex-col justify-end items-end shrink-0 pt-1">
                                    ...
                                    <div class="flex flex-col gap-2 pt-1 w-full">
                                        ...
                                        <button class="{{ $btnLt }} w-full"
                                            ...
                                            <span wire:loading.remove wire:target="openDetails({{ (int) $b->vehiclebooking_id }})">Detail</span>
                                        ...
                                        <button class="{{ $btnRed }} w-full"
                                            ...
                                            onclick="return confirm('Hapus sementara peminjaman ini?')"
                                            wire:loading.attr="disabled"
                                            wire:target="deleteBooking({{ (int) $b->vehiclebooking_id }})">
                                            <span wire:loading.remove wire:target="deleteBooking({{ (int) $b->vehiclebooking_id }})">Hapus</span>
                                        ...
                                        <button class="{{ $btnBlk }} !bg-emerald-600 hover:!bg-emerald-700 w-full"
                                            ...
                                            <span wire:loading.remove wire:target="restoreBooking({{ (int) $b->vehiclebooking_id }})">Pulihkan</span>
        ...
        <div class="fixed inset-0 z-[60] lg:hidden">
            ...
            <div class="absolute inset-x-0 bottom-0 bg-white rounded-t-2xl shadow-xl max-h-[80vh] overflow-hidden">
                ...
                <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                    <div>
                        <h3 class="text-base font-semibold text-gray-900">Pengaturan Filter</h3>
                        <p class="text-[11px] text-gray-500">Sesuaikan kriteria filter riwayat kendaraan.</p>
                    </div>
                    <button class="text-gray-500 hover:text-gray-700" type="button" wire:click="closeFilterModal" aria-label="Tutup">
                        <x-heroicon-o-x-mark class="w-5 h-5" />
                    </button>
                </div>
                ...
                <div class="p-4 space-y-5 max-h-[70vh] overflow-y-auto">
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-gray-800 mb-2 border-b pb-1">Cari</h4>
                        <div class="relative">
                            <input type="text" wire:model.live.debounce.400ms="q" placeholder="Tujuan, destinasi, peminjam…"
                                class="{{ $input }} pl-10 w-full placeholder:text-gray-400">
                            <x-heroicon-o-magnifying-glass class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        </div>
                    </div>
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-gray-800 mb-2 border-b pb-1">Filter per Kendaraan</h4>
                        <button type="button"
                            ...
                            <span>Semua Kendaraan ({{ count($vehicles) }})</span>
                            @if(is_null($vehicleFilter))
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-gray-800 mb-2 border-b pb-1">Filter per Tanggal</h4>
                        <div class="relative">
                            <input type="date" class="{{ $input }}" wire:model.live="selectedDate">
                        </div>
                    </div>
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-gray-800 mb-2 border-b pb-1">Urutan</h4>
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-hidden relative">
                <form wire:submit.prevent="saveBooking">
                    <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white z-10">
                        <h3 class="text-lg font-semibold text-gray-900">Edit Peminjaman Kendaraan #{{ $editingBookingId }}</h3>
                        <button class="text-gray-500 hover:text-gray-700" type="button" wire:click="closeEditModal" aria-label="Tutup">
                            <x-heroicon-o-x-mark class="w-6 h-6" />
                        </button>
                    </div>
                    ...
                    <div class="p-5 space-y-4 overflow-y-auto max-h-[calc(90vh-120px)]">
                        ...
                        <div>
                            <label class="{{ $label }}">Tujuan/Keperluan</label>
                        ...
                        <div>
                            <label class="{{ $label }}">Destinasi (Opsional)</label>
                        ...
                        <div>
                            <label class="{{ $label }}">Catatan/Catatan Admin (Opsional)</label>
                    ...
                    <div class="px-5 py-3 border-t border-gray-200 flex justify-end sticky bottom-0 bg-white z-10">
                        <button type="submit"
                            ...
                            <span wire:loading.remove wire:target="saveBooking">Simpan Perubahan</span>
