    ...
    <main class="px-4 sm:px-6 py-6 space-y-8">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-gray-900 to-black text-white shadow-2xl">
            ...
            <div class="relative z-10 p-6 sm:p-8">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <h2 class="text-lg sm:text-xl font-semibold">Manajemen Buku Tamu</h2>
        ...
        <section class="{{ $card }}">
            <div class="px-5 py-4 border-b border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <h3 class="text-base font-semibold text-gray-900 shrink-0">Tambah Entri Tamu Baru</h3>
                <div class="w-full sm:w-72 relative">
                    <input type="text" wire:model.live.debounce.400ms="search" class="{{ $input }} pl-10 shadow-sm"
                        placeholder="Cari nama / keperluan / telepon…">
            ...
            <form class="p-5" wire:submit.prevent="create">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
                    ...
                    <div class="md:col-span-1">
                        ...
                        <input type="text" wire:model.defer="name" class="{{ $input }}" placeholder="Nama pengunjung">
                    ...
                    <div class="md:col-span-1">
                        <label class="{{ $label }}">Petugas Keamanan</label>
                ...
                <div class="pt-5">
                    <button type="submit" class="{{ $btnBlk }} inline-flex items-center gap-2 h-10 px-5 rounded-xl text-sm font-medium" wire:loading.attr="disabled"
                        wire:target="create">
                        <span wire:loading.remove wire:target="create">Simpan Entri</span>
            ...
            <div class="p-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            ...
                            <div class="flex gap-2 justify-end pt-3 border-t border-gray-100">
                                @if(!$row->deleted_at)
                                    <button class="{{ $btnBlk }}"
                                        ...
                                        <span wire:loading.remove wire:target="openEdit({{ $row->guestbook_id }})">Ubah</span>
                                        <span wire:loading wire:target="openEdit({{ $row->guestbook_id }})">Memuat…</span>
                                    </button>
                                    <button class="{{ $btnRed }}" wire:click="delete({{ $row->guestbook_id }})"
                                        onclick="return confirm('Pindahkan ke tempat sampah?')" wire:loading.attr="disabled"
                                        wire:target="delete({{ $row->guestbook_id }})">
                                        <span wire:loading.remove wire:target="delete({{ $row->guestbook_id }})">Hapus</span>
                                        <span wire:loading wire:target="delete({{ $row->guestbook_id }})">Menghapus…</span>
                                    </button>
                                @else
                                    ...
                                    <button type="button" class="{{ $btnRed }}"
                                        ...
                                        onclick="return confirm('Hapus permanen?')" wire:loading.attr="disabled"
                    ...
                    @empty
                        <div class="col-span-full py-14 text-center text-gray-500 text-sm bg-white rounded-xl border border-gray-200 shadow-sm">Tidak ada data tamu ditemukan.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="fixed inset-0 z-[60] flex items-center justify-center" role="dialog" aria-modal="true"
                ...
                <div class="relative w-full max-w-xl mx-4 {{ $card }} focus:outline-none" tabindex="-1">
                    <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900">Ubah Entri Tamu</h3>
                        <button class="text-gray-500 hover:text-gray-700" type="button"
                            wire:click="$set('modalEdit', false)" aria-label="Tutup">
                    ...
                    <form class="p-5" wire:submit.prevent="update">
                        ...
                        <div class="mt-6 flex items-center justify-end gap-3 border-t border-gray-200 pt-4">
                            <button type="button" class="{{ $btnLite }} h-10 px-4"
                                wire:click="$set('modalEdit', false)">Batal</button>
